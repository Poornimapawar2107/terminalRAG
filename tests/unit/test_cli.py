...
# The cli __init__.py shadows the module name with `from .main import main`,
# so we must grab the actual module object from sys.modules.
import vector_rag.cli.main  # noqa: F401 — force module import
cli_mod = sys.modules["vector_rag.cli.main"]
...
def test_cmd_ingest_success(capsys, tmp_path):
    mock_cfg = MagicMock()
    mock_cfg.logging.level = "INFO"
    mock_cfg.logging.file = None

    mock_svc = MagicMock()
    mock_svc.ingest_path.return_value = IngestionResult(
        discovered=2, parsed=2, skipped=0,
        chunks_created=4, embeddings_generated=4, vectors_indexed=4,
    )
    mock_svc.vector_store.count.return_value = 4

    with patch.object(cli_mod, "Settings") as mock_settings, \
         patch.object(cli_mod, "setup_logging"), \
         patch.object(cli_mod, "DocumentService", return_value=mock_svc):
        mock_settings.load_from_yaml.return_value = mock_cfg
...
def test_cmd_list_with_docs(capsys):
    mock_svc = MagicMock()
    ...
    with patch.object(cli_mod, "Settings") as mock_settings, \
         patch.object(cli_mod, "setup_logging"), \
         patch.object(cli_mod, "DocumentService", return_value=mock_svc):
        mock_settings.load_from_yaml.return_value = MagicMock()
...
def test_cmd_list_no_docs(capsys):
    mock_svc = MagicMock()
    mock_svc.list_documents.return_value = []
...
def test_cmd_delete_by_filename(capsys):
...
def test_cmd_delete_not_found(capsys):
    mock_svc = MagicMock()
    mock_svc.list_documents.return_value = []
...
def test_main_catches_domain_errors(capsys):
    mock_cfg = MagicMock()
    mock_cfg.logging.level = "INFO"
    mock_cfg.logging.file = None
    ...
    with patch.object(cli_mod, "Settings") as mock_settings, \
         patch.object(cli_mod, "setup_logging"), \
         patch.object(cli_mod, "DocumentService") as mock_svc_cls:
        mock_settings.load_from_yaml.return_value = mock_cfg
        mock_svc_cls.return_value.ingest_path.side_effect = ConfigurationError(
            "bad config", hint="Fix YAML"
        )
